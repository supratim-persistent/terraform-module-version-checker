...
Note: For checking duplicate issues, the utility checks the list of open issue titles  with the string _Upgarde <module_name> to latest version_ as shown below since it creates the issues with the same title, so any issue created for module upgrade manually should stick to the same format to avoid duplications.
